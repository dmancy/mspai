...
//============================================================================
template <>
void Com_Server<double>::Initialize_nbr_done(void)
{
    nbr_done = 0;
}

template <>
void Com_Server<double>::Initialize_nbr_done_prefetching(void)
{
...
void Com_Server<double>::Handle_Get_Col(Matrix<double>* A, const int requestor)
{
    ...
    if (MPI_Recv(static_cast<void*>(&idx), 1, MPI_INT, requestor, get_rc_tag, world, &status))
    ...
    if (MPI_Send(static_cast<int*>(col_idcs_buf), len_col, MPI_INT, requestor,
                 send_cols_tag, world))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
    ...
    if (MPI_Send(static_cast<double*>(buffer), slen_col, MPI_DOUBLE, requestor,
                 send_vals_tag, world))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Get_Col\" by MPI_Send.\n");
...
template <>
void Com_Server<double>::Handle_Get_Target_Col(const Matrix<double>* B, const int requestor)
{
    int idx, len_col = 0, *col_idcs_buf = NULL;
    ...
    // Local pe is receiving column index of the
    // column which another pe is requesting remotely.
    if (MPI_Recv(static_cast<void*>(&idx), 1, MPI_INT, requestor, get_Bc_tag, world, &status))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
    ...
    // Sending column indices
    if (MPI_Send(static_cast<int*>(col_idcs_buf), len_col, MPI_INT, requestor,
                 send_Bcols_tag, world))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Get_Target_Col\" by MPI_Send.\n");

    // Sending column values
    if (MPI_Send(static_cast<double*>(col_buf), len_col, MPI_DOUBLE, requestor,
                 send_Bvals_tag, world))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Get_Target_Col\" by MPI_Send.\n");
}
...
template <>
void Com_Server<double>::Handle_Insert_Row_Solution(Matrix<double>* M, int requestor)
{
    int idx_and_len[3], idx, nnz, *col_idcs_buf = NULL;
    ...
    // to store
    if (MPI_Recv(static_cast<void*>(idx_and_len), 3, MPI_INT, requestor,
                 put_Mcol_tag, M->world, &status))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
    ...
    if (MPI_Recv(static_cast<void*>(col_idcs_buf), nnz, MPI_INT, requestor,
                 put_Mcol_inds_tag, M->world, &status))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Insert_Row_Solution\" by MPI_Recv.\n");
    ...
    if (MPI_Recv(static_cast<void*>(col_buf), nnz, MPI_DOUBLE, requestor,
                 put_Mcol_vals_tag, M->world, &status))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Insert_Row_Solution\" by MPI_Recv.\n");
...
template <>
void Com_Server<double>::Get_Remote_Col(Matrix<double>* A,
                                        Matrix<double>*& M,
                                        Matrix<double>* B,
                                        Pattern* P,
                                        Pattern* UP,
                                        ...
                                        int& idx)
{
    int send_flag, cols_flag, rows_flag, vals_flag, scalar_len = 0;

    MPI_Status send_status, cols_status, rows_status, vals_status;

    MPI_Request get_cols_request, get_rows_request, get_vals_request, send_request;
    ...
    if (MPI_Irecv(static_cast<void*>(A->remote_col_idcs_buf), A->max_nnz,
                  MPI_INT, pe, send_cols_tag, world, &get_cols_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Col\" by MPI_Irecv.\n");

    if (A->c_lines->row_idcs) {
        if (MPI_Irecv(static_cast<void*>(A->remote_row_idcs_buf), A->max_nnz,
                      MPI_INT, pe, send_rows_tag, world, &get_rows_request))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
                "Get_Remote_Col\" by MPI_Irecv.\n");
    }

    if (MPI_Irecv(static_cast<void*>(A->remote_col_buf), A->max_nnz, MPI_DOUBLE,
                  pe, send_vals_tag, world, &get_vals_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Col\" by MPI_Irecv.\n");

    // Send the request
    if (MPI_Isend(static_cast<void*>(&idx), 1, MPI_INT, pe, get_rc_tag, world, &send_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
...
template <>
void Com_Server<double>::Get_Remote_Target_Col(Matrix<double>* A,
                                               Matrix<double>*& M,
                                               Matrix<double>* B,
                                               Pattern* P,
                                               Pattern* UP,
                                               ...
                                               int& idx)
{
    int send_flag, cols_flag, vals_flag;

    MPI_Status send_status, cols_status, vals_status;

    MPI_Request get_cols_request, get_vals_request, send_request;
    ...
    // Element is not in cache - get it from a remote pe
    if (MPI_Irecv(static_cast<void*>(B->remote_col_idcs_buf), B->max_nnz,
                  MPI_INT, pe, send_Bcols_tag, world, &get_cols_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Target_Col\" by MPI_Irecv.\n");

    if (MPI_Irecv(static_cast<void*>(B->remote_col_buf), B->max_nnz, MPI_DOUBLE,
                  pe, send_Bvals_tag, world, &get_vals_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Target_Col\" by MPI_Irecv.\n");

    // Send the request
    if (MPI_Isend(static_cast<void*>(&idx), 1, MPI_INT, pe, get_Bc_tag, world, &send_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
...
template <>
void Com_Server<double>::Insert_Row_Solution(Matrix<double>* A,
                                             Matrix<double>*& M,
                                             Matrix<double>* B,
                                             Pattern* P,
                                             Pattern* UP,
                                             ...
                                             const Index_Set* J)
{
    int pe, idx, nnz, *col_idcs_buf = NULL, flag, idx_and_len[3];
    ...
    else // The line is remote.
    {
        ...
        // which will be sent.
        if (MPI_Isend(static_cast<void*>(idx_and_len), 3, MPI_INT, pe,
                      put_Mcol_tag, A->world, &requests[0]))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
        ...
        if (MPI_Isend(static_cast<void*>(J->idcs), nnz, MPI_INT, pe,
                      put_Mcol_inds_tag, A->world, &requests[1]))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
                "Insert_Row_Solution\" by MPI_Isend.\n");

        if (MPI_Isend(static_cast<void*>(mk_Hat), nnz, MPI_DOUBLE, pe,
                      put_Mcol_vals_tag, A->world, &requests[2]))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
                "Insert_Row_Solution\" by MPI_Isend.\n");
...
template <>
void Com_Server<double>::Insert_Row_Solution_Block(Matrix<double>* A,
                                                   Matrix<double>*& M,
                                                   Matrix<double>* B,
                                                   Pattern* P,
                                                   Pattern* UP,
                                                   ...
                                                   const Index_Set* J)
{
    int pe, idx, nnz,
        scalar_nnz, // number of scalar entries in the M column
            *col_idcs_buf = NULL, flag, idx_and_len[3];
    ...
    if (pe == A->my_id) // The line is local.
    {
        ...
        {
            col_idcs_buf[i] = J->idcs[i];
            for (int k = 0; k < A->block_sizes[J->idcs[i]]*A->block_sizes[col];
        k++)
            {
...
template <>
void Com_Server<COMPLEX>::Handle_Get_Col(const Matrix<COMPLEX>* A, const int requestor)
{
    int idx, len_col = 0, len_row = 0, *col_idcs_buf = NULL, *row_idcs_buf = NULL;
...
template <>
void Com_Server<COMPLEX>::Handle_Get_Target_Col(const Matrix<COMPLEX>* B, const int requestor)
{
    int idx, len_col = 0, *col_idcs_buf = NULL;
    ...
    // Sending column values
    if (MPI_Send(static_cast<COMPLEX*>(col_buf), len_col, COMPLEX_MPI,
                 requestor, send_Bvals_tag, world))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Get_Target_Col\" by MPI_Send.\n");
}
...
template <>
void Com_Server<COMPLEX>::Handle_Insert_Row_Solution(Matrix<COMPLEX>* M, int requestor)
{
    int idx_and_len[3], idx, nnz, *col_idcs_buf = NULL;
    ...
    if (MPI_Recv(static_cast<void*>(col_buf), nnz, COMPLEX_MPI, requestor,
                 put_Mcol_vals_tag, M->world, &status))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Handle_Insert_Row_Solution\" by MPI_Recv.\n");
...
template <>
void Com_Server<COMPLEX>::Get_Remote_Col(Matrix<COMPLEX>* A,
                                         Matrix<COMPLEX>*& M,
                                         Matrix<COMPLEX>* B,
                                         Pattern* P,
                                         Pattern* UP,
                                         ...
                                         int& idx)
{
    int send_flag, cols_flag, rows_flag, vals_flag;
    ...
    if (MPI_Irecv(static_cast<void*>(A->remote_col_buf), A->max_nnz,
                  COMPLEX_MPI, pe, send_vals_tag, world, &get_vals_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Col\" by MPI_Irecv.\n");
...
template <>
void Com_Server<COMPLEX>::Get_Remote_Target_Col(Matrix<COMPLEX>* A,
                                                Matrix<COMPLEX>*& M,
                                                Matrix<COMPLEX>* B,
                                                Pattern* P,
                                                Pattern* UP,
                                                ...
                                                int& idx)
{
    ...
    if (MPI_Irecv(static_cast<void*>(B->remote_col_buf), B->max_nnz,
                  COMPLEX_MPI, pe, send_Bvals_tag, world, &get_vals_request))
        throw std::runtime_error(
            "\n\tERROR in method \"Com_Server::"
            "Get_Remote_Target_Col\" by MPI_Irecv.\n");
...
template <>
void Com_Server<COMPLEX>::Insert_Row_Solution(Matrix<COMPLEX>* A,
                                              Matrix<COMPLEX>*& M,
                                              Matrix<COMPLEX>* B,
                                              Pattern* P,
                                              Pattern* UP,
                                              ...
                                              const Index_Set* J)
{
    ...
    else // The line is remote.
    {
        ...
        // Prepare remote pe for index of column and length of data
        // which will be sent.
        if (MPI_Isend(static_cast<void*>(idx_and_len), 2, MPI_INT, pe,
                      put_Mcol_tag, A->world, &requests[0]))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
                "Insert_Row_Solution\" by MPI_Isend.\n");
        ...
        if (MPI_Isend(static_cast<void*>(mk_Hat), nnz, COMPLEX_MPI, pe,
                      put_Mcol_vals_tag, A->world, &requests[2]))
            throw std::runtime_error(
                "\n\tERROR in method \"Com_Server::"
                "Insert_Row_Solution\" by MPI_Isend.\n");
