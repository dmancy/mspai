...
void
Index_Set::Print_Index_Set(char* str)
{
    std::cout <<"\t" << str <<  " ";
    for (int i = 0; i < len; i++)
            std::cout << idcs[i] << " ";
...
Index_Set* 
Index_Set::Set_Difference(Index_Set* is_a, 
                          Index_Set* is_b)
{
    int     a,
            b,
...
Index_Set*
Index_Set::Set_Intersection(Index_Set*  is_a, 
                            Index_Set*  is_b)
{
    int         a,
                b,
...
int
Index_Set::Get_El_Idx(Index_Set* is, const int el)
{
    for (int idx = 0; idx < is->len; idx++)
        if (el == is->idcs[idx]) return idx;
    return -1;
}
