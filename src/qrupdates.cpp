...
qrs* QRZerlegungLapack(double *A,double *b,int zeilen, int spalten, int dimAz, int dimAs, int *gewaehlteZeilen, int *gewaehlteSpalten, int max_pattern_updates, int max_pattern){
    int index, i, offset, info, worksize, worksize2, nrhs, *bs, zeilen_dlarf, spaltenindex, eins;
    double *bpuffer, *x_feld, *tau, *work, *R_HH, *b_perm, *b_perm2, *my_b_perm, diag_sicherung, beta_wert;
    const char *seite = "L", *trans = "T", *uplo = "U", *diag = "N", *trans2 = "N";
    qrs *QR;
    ...
    spermutation = (permelement*) malloc((spalten+max_pattern*max_pattern_updates+1)*sizeof(permelement));    
    x_feld = (double*) malloc((zeilen*spalten + dimAz*(max_pattern*max_pattern_updates+1))*sizeof(double));
    ...
    QR->hinzugefuegte_spalten = (int*)malloc((spalten + max_pattern_updates)*sizeof(int));
    QR->hinzugefuegte_zeilen = (int*)malloc((zeilen + max_pattern_updates)*sizeof(int));
...
//Zerlegung der Matrix
    worksize = getOptWork_dgeqrf(A, zeilen, spalten); /* Bestimmen des optimalen temp. Speichers f�r die Zerlegung  */
    work = (double*)malloc(worksize*sizeof(double));
    dgeqrf_(&zeilen,&spalten,A,&zeilen,tau,work,&worksize,&info); /* Zerlegung der Matrix A enthaelt R und die Householder-Vektoren */
    if (info != 0){
           printf("dgeqrf info = %d\n",info);
     }
     worksize2 = getOptWork_dormqr(seite,trans,zeilen,nrhs,A,tau,b); /* Bestimmen des optimalen temp. Speichers f�r  Q'*b*/
...
//Struktur anpassen
     memcpy(x_feld, A, zeilen*spalten*sizeof(double)); /* Kopieren der Zerlegung */
...
qrs *qrupdate_voll(qrs* QR, double *x, double *b, int neueSpalten, int neueZeilen, const int *neuespalten, const int *neuezeilen){

    int laenge, *bs, ahut_update_zeilen, dim_aenderung, anzahl_updates, zeilen_dlarf, eins, spaltenindex, info, B2_zeilen, dims, A_zeilen, i, j, k, ahut_zeilen, *perm, worksize, index, ahut_spalten;
    const char *seite = "L";
    double *R_HH2, *work, *tau, *R_HH, diag_sicherung, *b_perm, *b_perm2, *beta, beta_wert, zwischenergebnis;
    permelement *spermutation;
    ...
    R_HH = QR->R_HH;
    index = QR->R_HH_Pegel;
//anhaengen der neuen permutierten Vektoren an die Matrix / Sichern der Spaltenpermutation
...
// b um neue Zeilen erweitern
    ...
    worksize = getOptWork_dgeqrf(R_HH, B2_zeilen, neueSpalten); /* Bestimmen des optimalen temp. Speichers f�r die Zerlegung  */
...
//Q' anwenden auf den Bereich der neuen Spalten, der keine neuen Zeilen hat
    ...
    for (i = 0; i <= anzahl_updates; i++){
             ...
             for(k = dims; k < dim_aenderung; k++){
                     ...
                     *R_HH = 1;
                     dlarf_(seite, &zeilen_dlarf, &neueSpalten, R_HH, &eins, &beta_wert, R_HH2, &laenge, work); /*Anwenden der einzelnen vektoren auf den neuen Vektor */
                     *R_HH = diag_sicherung;
                     R_HH2++;
                     spaltenindex++;
...
//Anwenden der Q' auf b
    ...
    for (i = 0; i <= anzahl_updates ; i++){
             ...
             for(k = dims; k < dim_aenderung; k++){
                     ...
                     beta_wert = beta[spaltenindex];
                     *R_HH = 1;
                     dlarf_(seite, &zeilen_dlarf, &eins, R_HH, &eins, &beta_wert, b_perm2, &ahut_update_zeilen, work);       /*Anwenden der n Householdervektoren auf den b - Vektor */
                     *R_HH = diag_sicherung;
                     b_perm2++;
                     spaltenindex++;
                     R_HH += (ahut_update_zeilen + 1);
...
qrs_sparse* QRZerlegung_cs(cs *Matrix, double *b, int zeilen, int spalten, int nnz, int dimAz, int dimAs, int *gewaehlteZeilen, int *gewaehlteSpalten, int max_pattern_updates, int max_pattern){
...
// Problem loesen Besetztheit < 15% dann AMD ansonsten urspruengliche Anordnung
    order = (double)nnz / (zeilen * spalten) > 0.15 ? 0 : 3;
    S = cs_sqr (order, Matrix, 1) ;     /* Analyse der Matrix */
    anzahl_xi_guess = ((int)(S->lnz)/spalten) * (max_pattern * (max_pattern_updates + 1));
...
// Q' auf b anwenden
        ...
        for (i = 0 ; i < spalten ; i++){            /* anwenden der  Householder refl. auf x */
...
qrs_sparse *qrupdate_duenn(qrs_sparse* QR, double *x, double *b, int neueSpalten, int neueZeilen, const int *neuespalten, const int *neuezeilen){
   double *myb,*beta, *tmpx, wert,*uutmpx;
   double *Lx, *x2, *x1, *Rx;
   int B2_laenge, *Li, *Lp, *Ri, *uutmpi, *uutmpp, *Rp, letzte_p, k, *tmpp, *tmpi, index, ahut_zeilen, ahut_spalten, j, zaehler, zaehler2, zaehler3, i, A_zeilen, *perm, platz;
...
//Q'x berechnen und B2 herstellen
   ...
   for (i = 0; i < neueSpalten; i++){
            zaehler3 = 0;
            for (k = 0 ; k < ahut_spalten ; k++){       /* anwenden der  Householder refl. auf x */
...
//analyse 
   ...
   if(QR->size_HH - S->lnz < 0){
      	platz = QR->L->nzmax + (((int)S->lnz / neueSpalten) * (QR->updates + 1) * QR->pattern);
...
//Anwenden von Q' auf b
   for (k = 0 ; k < QR->ahut_spalten ; k++){           /* anwenden der  Householder refl. auf x */
            cs_happly (QR->L, k, QR->beta [k], x1) ;
   }
//L�sen des neuen Gleichungssystemes
   cs_usolve (QR->U, x1) ;     /* x = R\x */
   if(QR->q){
          cs_ipvec (QR->q, x1, QR->amdpuffer , QR->ausgang_ahut_spalten);       /* b(q(0:n-1)) = x(0:n-1) */
...
qrs_sparse *qrupdate_cs_lapack(qrs_sparse *QR, double *x, double *b, int neueSpalten, int neueZeilen, const int *neuespalten, const int *neuezeilen){ 
	int *pinv,B2_zeilen, letzte_p, letzte_p2, platz, zaehler, zaehler2, *Lp, *Li, *Rp, *Ri, laenge, info, A_zeilen, i, j, k, ahut_zeilen, *perm, worksize, ahut_spalten;
	...
	tau = QR->beta+ahut_spalten;
	x2 += ahut_spalten;
        worksize = getOptWork_dgeqrf(x2, B2_zeilen, neueSpalten); /* Bestimmen des optimalen temp. Spenge * neueSpalten)eichers f�r die Zerlegung  */
        work = (double*)malloc(worksize*sizeof(double));
	...
	if(QR->size_HH - ((laenge - ahut_spalten + 1) * neueSpalten) < 0){
	        platz = QR->L->nzmax + ((laenge - ahut_spalten + 1) * neueSpalten * (QR->updates + 1));
	        QR->HH_i = (int*)realloc(QR->HH_i, platz * sizeof(int));
	        QR->HH_x = (double*)realloc(QR->HH_x, platz * sizeof(double));
	        QR->L->x = QR->HH_x;
	        QR->L->i = QR->HH_i;
...
int getOptWork_dormqr(const char *seite, const char* trans, int zeilen, int nhrs, double *A, double *tau, double *b){
        int lw = -1,info = 0;
        double w = 0.0;
        dormqr_(seite, trans, &zeilen, &nhrs, &zeilen, A, &zeilen, tau, b, &zeilen, &w, &lw, &info);
        return (int)w;
}
...
int vergleicher(const void* a, const void* b){
        if( ((permelement*)a)->aspalte < ((permelement*)b)->aspalte ) return -1;
	else return 1;
}
