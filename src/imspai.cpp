...
PetscErrorCode PC_MSPAI::PCSetUp_MSPAI(Mat A)
{
    ...
    B = Matrix<double>::Convert_Block_Matrix(A_REAL, 2, 10000,0);
     ...
     //B->Print_Matrix_Data(B);
      //A_REAL->Print_Matrix_Data(A_REAL);
...
{
	PetscErrorCode ierr;
	ierr = MatMult(*PM, xx, yy);CHKERRQ(ierr);
	return 0;
}

//PetscErrorCode PC_MSPAI::PCSetFromOptions_SPAI(PetscOptionItems *PetscOptionsObject,PC pc)
PetscErrorCode PC_MSPAI::PCSetFromOptions_SPAI(PetscOptionItems *PetscOptionsObject)
{
	...
	PetscFunctionBegin;
//	PetscOptionsBegin(PETSC_COMM_WORLD, "", "Options for the MSPAI Preconditioner", "MSPAI");
	ierr = PetscOptionsHead(PetscOptionsObject,"MSPAI options");CHKERRQ(ierr);
	//ierr = PetscOptionsReal("-ep","","PCSetEpsilon",epsilon_param,&ep,&flg);CHKERRQ(ierr);
    ...
    if (ch < 0 || ch > 1)
    {
        out_str << ch;
        throw std::runtime_error(
            "\n\tIllegal hash size parameter: " 
                + out_str.str() + "\n"
            "\tPlease use 1 for using a hashtable and 0 for not using a hashtable.\n"
            "\tdefault is ch = 0\n");
    }
    ...
    if (um < 0 || um > 1)
    {
        out_str << um;
        throw std::runtime_error(
            "\n\tIllegal value for using mean value: " 
                + out_str.str() + "\n"
            "\tAllowed are:\n"
            "\t\t0: don't use mean value as bound for augmenting indices\n"
            "\t\t1: use mean value as bound for augmenting indices (default)\n");
    }
    ...
    if (pk < 0)
    {
        out_str << pk;
        throw std::runtime_error(
            "\n\tIllegal number of prerequesting columns k: " 
            + out_str.str() +  "\n"
            "\tAllowed are positive values.\n");
    }
    ...
    if ((probing_Ce_file && !probing_Be_file) || 
        (!probing_Ce_file && probing_Be_file))
        throw std::runtime_error(
