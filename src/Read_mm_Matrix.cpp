...
int Read_mm_Matrix::Get_Matrix_Type(char* matrix_file, bool& symmetric, bool& hermitian)
{
    int type;
...
void Read_mm_Matrix::Read_Header(FILE* f, int& type, bool& symmetric, bool& hermitian)
{
    MM_typecode matcode;
...
void Read_mm_Matrix::Skip_Header(FILE* f)
{
    int m, n, nnz, dummy1;
...
void Read_mm_Matrix::Read_Matrix_File(char* matrix_file,
                                      char* probing_file,
                                      const bool use_prob,
                                      const int use_schur,
                                      bool& symmetric,
                                      ...
                                      MPI_Comm world)
{
    int file_M = -1, file_M_prob = 0, file_N = -1, file_N_prob = 0,
        file_nnz = -1, file_nnz_prob = 0, M, N, nnz, nnz_cols, nnz_rows,
        start_idx, split_idx, split_pe, my_nbr_cols, my_nbr_rows, prob_type = -1;

    bool symmetric_prob = false, hermitian_prob = false;

    FILE *f = NULL, *f_prob = NULL;

    RCV *cols = NULL, *rows = NULL;
    ...
    if (use_prob) {
        prob_type = Get_Matrix_Type(probing_file, symmetric_prob, hermitian_prob);
        ...
        o_mmio.MM_Read_Mtx_Crd_Size(f_prob, file_M_prob, file_N_prob, file_nnz_prob);
        ...
        if (use_schur) {
            if (file_N_prob >= file_N)
                throw std::runtime_error(
                    "\n\tERROR:  Mismatch in matrix and schur probing "
                    "dimensions!\n"
    ...
    o_dist.Basic_Distribution(world, N, my_nbr_cols, split_pe, split_idx, start_idx);

    Read_Matrix_Data(f, f_prob, M, nnz, file_N, file_nnz, file_N_prob,
                     file_nnz_prob, my_nbr_cols, my_nbr_rows, start_idx, &cols,
                     &rows, &nnz_cols, &nnz_rows, rho_param, symmetric,
    ...
    A = Data_To_Matrix(N, M, my_nbr_cols, cols, rows, nnz_cols, nnz_rows,
                       symmetric, M, world);
...
void Read_mm_Matrix::Read_Target_File(Matrix<double>* A,
                                      char* matrix_file,
                                      char* probing_file,
                                      const bool use_prob,
                                      const int use_schur,
                                      ...
                                      MPI_Comm world)
{
    int file_M_prob = 0, file_N_prob = 0, file_nnz_prob = 0, M, N, nnz_cols,
        nnz_rows, start_idx, split_idx, split_pe, my_nbr_cols, my_nbr_rows,
        prob_Ce_M = 0, dim_zero = 0, prob_type = -1;

    bool symmetric_system = false, symmetric_prob = false,
         hermitian_system = false, hermitian_prob = false;
    ...
    if (use_schur) {
        ...
        Generate_Identity_Data(f_prob, N, start_idx, file_nnz_prob, dim_zero,
                               my_nbr_cols, my_nbr_rows, &cols, &rows, &nnz_cols,
                               &nnz_rows, rho_param, symmetric_prob, hermitian_prob);

        if (use_prob)
            fclose(f_prob);

        B = Data_To_Matrix(N, M, my_nbr_cols, cols, rows, nnz_cols, nnz_rows,
                           symmetric_system, M - dim_zero, world);
        ...
        if (!target_param) {
            ...
            Read_Matrix_File(matrix_file, probing_file, use_prob, use_schur, symmetric_system,
                             hermitian_system, B, prob_Ce_N, rho_param, world);
        ...
        else // generate identity target matrix
        {
            ...
            Generate_Identity_Data(f_prob, N, start_idx, file_nnz_prob, dim_zero, my_nbr_cols,
                                   my_nbr_rows, &cols, &rows, &nnz_cols, &nnz_rows,
            ...
            B = Data_To_Matrix(N, M, my_nbr_cols, cols, rows, nnz_cols,
                               nnz_rows, symmetric_system, M, world);
...
void Read_mm_Matrix::Read_Matrix_Data(FILE* f,
                                      FILE* f_prob,
                                      int M,
                                      int nnz,
                                      int file_N,
...
{
    int my_nnz_col = 0, my_nnz_row = 0, lenrow = 0, lencol = 0, prob_diff = 0;

    RCV *tmp_cols, *tmp_rows;

    // First parse through files to precompute
    // memory space to be allocated
    Count_NNZ(f, my_nnz_col, my_nnz_row, my_nbr_cols, my_nbr_cols, file_nnz,
              start_idx, 0, symmetric, hermitian);
    if (f_prob) {
        ...
        Count_NNZ(f_prob, my_nnz_col, my_nnz_row, my_nbr_cols, my_nbr_rows,
                  file_nnz_prob, start_idx, prob_diff, symmetric_prob, hermitian_prob);
    }
    ...
    *rows = tmp_rows;
    ...
    Data_To_Memory(f, my_nbr_cols, my_nbr_cols, file_nnz, start_idx, lencol,
                   lenrow, tmp_cols, tmp_rows, 0, 1.0, 0, symmetric, hermitian);

    if (f_prob) {
        rewind(f_prob);
        Skip_Header(f_prob);
        Data_To_Memory(f_prob, my_nbr_cols, my_nbr_rows, file_nnz_prob,
                       start_idx, lencol, lenrow, tmp_cols, tmp_rows, prob_diff,
                       rho_param, file_N, symmetric_prob, hermitian_prob);
    }
}

void Read_mm_Matrix::Count_NNZ(FILE* f,
                               int& my_nnz_col,
                               int& my_nnz_row,
...
void Read_mm_Matrix::Data_To_Memory(FILE* f,
                                    const int my_nbr_cols,
                                    const int my_nbr_rows,
                                    const int file_nnz,
                                    const int start_idx,
...
void Read_mm_Matrix::Generate_Identity_Data(FILE* f_prob,
                                            int N,
                                            int start_idx,
                                            int file_nnz_prob,
                                            const int dim_zero,
                                            ...
                                            const bool hermitian_prob)
{
    int my_nnz_col = 0, my_nnz_row = 0, lenrow = 0, lencol = 0, dim = 0;
    ...
    if (f_prob) {
        ...
        // be adjusted
        Count_NNZ(f_prob, my_nnz_col, my_nnz_row, my_nbr_cols, my_nbr_rows,
                  file_nnz_prob, start_idx, 0, symmetric_prob, hermitian_prob);
    }
...
Matrix<double>* Read_mm_Matrix::Data_To_Matrix(int N,
                                               int M,
                                               int my_nbr_cols,
                                               RCV* cols,
                                               RCV* rows,
                                               ...
                                               MPI_Comm world)
{
    int *nnz_per_col = NULL, *nnz_per_row = NULL, start_idx, col_buf_size = 0,
        row_buf_size = 0, A_buf_size = 0, idx = 0, len_col, len_row, gap = 0, max = 0;
    ...
    MPI_Allgather(static_cast<void*>(&my_nbr_cols), 1, MPI_INT,
                  static_cast<void*>(mtx->all_nbr_cols), 1, MPI_INT, world);

    // Filling start indices
    mtx->start_indices[0] = 0;
    for (int pe = 1; pe < mtx->num_procs; pe++)
        mtx->start_indices[pe] = mtx->start_indices[pe - 1] + mtx->all_nbr_cols[pe - 1];
    ...
    if (!symmetric) {
        ...
        Count_NNZ_Rows(rows, (size_t)nnz_rows, nnz_per_row);
    }

    mtx->my_nnz = nnz_cols;
    mtx->c_lines = new Compressed_Lines<double>(mtx->my_nbr_cols, symmetric);
    lines = mtx->c_lines;
    ...
    MPI_Barrier(world);
    MPI_Allgatherv(static_cast<void*>(lines->len_cols), mtx->my_nbr_cols,
                   MPI_INT, static_cast<void*>(mtx->len_all_cols),
                   mtx->all_nbr_cols, mtx->start_indices, MPI_INT, world);

    // Get length of all rows
    mtx->len_all_rows = new int[mtx->n];
    MPI_Barrier(world);
    MPI_Allgatherv(static_cast<void*>(lines->len_rows), mtx->my_nbr_cols,
                   MPI_INT, static_cast<void*>(mtx->len_all_rows),
                   mtx->all_nbr_cols, mtx->start_indices, MPI_INT, world);
    ...
    MPI_Barrier(world);
    MPI_Allreduce(&max, &mtx->max_nnz, 1, MPI_INT, MPI_MAX, world);
...
void Read_mm_Matrix::Count_NNZ_Cols(RCV const* in, size_t size, int* out)
{
...
void Read_mm_Matrix::Count_NNZ_Rows(RCV const* in, size_t size, int* out)
{
    // Columns with no nnz will be present with
    // no entry in out
    int diff = 0;
...
void Read_mm_Matrix::Read_Target_File(Matrix<COMPLEX>* A,
                                      char* matrix_file,
                                      char* probing_file,
                                      const bool use_prob,
                                      const int use_schur,
...
Matrix<COMPLEX>* Read_mm_Matrix::Data_To_Matrix(int N,
                                                int M,
                                                int my_nbr_cols,
                                                RCC* cols,
                                                RCC* rows,
                                                ...
                                                MPI_Comm world)
{
    ...
    mtx->my_nnz = nnz_cols;
    mtx->c_lines = new Compressed_Lines<COMPLEX>(mtx->my_nbr_cols, symmetric);
    lines = mtx->c_lines;
...
void Read_mm_Matrix::Count_NNZ_Cols(RCC const* in, size_t size, int* out)
{
    // Columns with no nnz will be present with
    // no entry in out
    int diff = 0;
...
void Read_mm_Matrix::Count_NNZ_Rows(RCC const* in, size_t size, int* out)
{
    // Columns with no nnz will be present with
    // no entry in out
    int diff = 0;
